...
/** Pair 1 (Families C/D web setup 2–3): master publishes `webEmotionRxLocked` when peer has sa=1. */
async function fetchWebEmotionRxLocked(token, deviceId, setupIndex) {
  if (!token || !deviceId) return false;
  if (setupIndex !== 2 && setupIndex !== 3) return false;
  try {
    const resp = await fetch(
      `https://api.particle.io/v1/devices/${deviceId}/webEmotionRxLocked?access_token=${encodeURIComponent(token)}`
    );
...
export default async function handler(req, res) {
  ...
  let webEmotionRxLocked = false;
  if (token && masterId && masterOnline && masterResult.online !== false) {
    webEmotionRxLocked = await fetchWebEmotionRxLocked(token, masterId, setup);
  }

  const out = { master: masterOnline, light: lightOnline, sound: soundOnline, webEmotionRxLocked };
  if (debug) {
    out._debug = {
